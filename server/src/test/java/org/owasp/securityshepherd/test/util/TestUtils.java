...
public final class TestUtils {
	...
	public static final Long[] LONGS = { INITIAL_LONG, 1L, -1L, 100L, -100L, 1000L, -1000L,
			Long.valueOf(Integer.MAX_VALUE) + 1, Long.valueOf(Integer.MIN_VALUE - 1), 123456789L, -12346789L,
	...
	public static final LocalDateTime[] LOCALDATETIMES = { INITIAL_LOCALDATETIME, INITIAL_LOCALDATETIME.plusNanos(1),
			INITIAL_LOCALDATETIME.plusSeconds(1), INITIAL_LOCALDATETIME.plusMinutes(1),
			INITIAL_LOCALDATETIME.plusHours(1), INITIAL_LOCALDATETIME.plusDays(1), INITIAL_LOCALDATETIME.plusWeeks(1),
			INITIAL_LOCALDATETIME.plusMonths(1), INITIAL_LOCALDATETIME.plusYears(1),
			INITIAL_LOCALDATETIME.plusYears(1000), INITIAL_LOCALDATETIME.plusYears(100000), LocalDateTime.MAX };

	public static final LocalDateTime[] LOCALDATETIMES_WITH_NULL = (LocalDateTime[]) ArrayUtils.addAll(LOCALDATETIMES,
			(LocalDateTime) null);

	public static final String INITIAL_STRING = "";

	public static final String[] STRINGS = { INITIAL_STRING, "Test String", "åäö", "me@example.com",
			"1;DROP TABLE users", " ", "%", "_", "-", "--", "జ్ఞ‌ా", "Ｔｈｅ ｑｕｉｃｋ ｂｒｏｗｎ ｆｏｘ ｊｕｍｐｓ ｏｖｅｒ ｔｈｅ ｌａｚｙ ｄｏｇ",
			...
			"בְּרֵאשִׁי  בָּרָא אֱלֹהִים, אֵת הַשָּׁמַיִם, וְאֵת        הָיְתָהtest", " ﷽ ", "ﷺ",
			"مُنَاقَشَةُ سُبُلِ اِسْتِخْدَامِ اللُّغَةِ فِي النُّظُمِ الْقَائِمَةِ وَفِيم يَخُصَّ التَّطْبِيقَاتُ الْحاسُوبِيَّةُ، " };

	public static final String[] STRINGS_WITH_NULL = (String[]) ArrayUtils.addAll(STRINGS, (String) null);
	...
	public static final byte[][] BYTE_ARRAYS_WITH_NULL = (byte[][]) ArrayUtils.addAll(BYTE_ARRAYS, (byte[]) null);
