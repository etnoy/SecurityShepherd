...
public class UserRepositoryTest {
	...
	public void existsById_ExistingId_ReturnsTrue() {
		...
		User returnedUser = userRepository.save(existsByIdExistingIdUser).block();

		assertNotNull(returnedUser.getId());
		assertTrue(userRepository.existsById(returnedUser.getId()).block());
	...
	public void existsById_NonExistentId_ReturnsFalse() {
		...
		assertFalse(userRepository.existsById(1234567890).block());
	...
	public void count_KnownNumberOfUsers_ReturnsCorrectNumber() {
		...
		assertEquals(0, userRepository.count().block());

		userRepository.save(User.builder().displayName("count_KnownNumberOfUsers1").build());
		assertEquals(1, userRepository.count().block());
	...
	public void save_ValidUser_ContainedInAllUsers() {
		...
		validUser1 = userRepository.save(validUser1).block();
		validUser2 = userRepository.save(validUser2).block();
		validUser3 = userRepository.save(validUser3).block();

		List<User> allUsers = (List<User>) userRepository.findAll().collectList().block();
	...
	public void deleteById_ValidId_DeletesUser() {
		...
		User returnedUser = userRepository.save(User.builder().displayName("deleteById_ValidId").build()).block();

		userRepository.deleteById(returnedUser.getId());

		assertFalse(userRepository.existsById(returnedUser.getId()).block());
	...
	public void findAll_ReturnsUsers() {
		...
		assertTrue(userRepository.count().block() == 0);

		User findAll_ReturnsUsers_user1 = userRepository
				.save(User.builder().displayName("findAll_ReturnsUsers_user1").build()).block();
		User findAll_ReturnsUsers_user2 = userRepository
				.save(User.builder().displayName("findAll_ReturnsUsers_user2").build()).block();
		User findAll_ReturnsUsers_user3 = userRepository
				.save(User.builder().displayName("findAll_ReturnsUsers_user3").build()).block();
		User findAll_ReturnsUsers_user4 = userRepository
				.save(User.builder().displayName("findAll_ReturnsUsers_user4").build()).block();

		assertTrue(userRepository.existsById(findAll_ReturnsUsers_user1.getId()).block());
		assertTrue(userRepository.existsById(findAll_ReturnsUsers_user2.getId()).block());
		assertTrue(userRepository.existsById(findAll_ReturnsUsers_user3.getId()).block());
		assertTrue(userRepository.existsById(findAll_ReturnsUsers_user4.getId()).block());

		List<User> users = (List<User>) userRepository.findAll().collectList().block();
	...
	public void findById_NonExistentId_ReturnsNull() {
		...
		assertNull(userRepository.findById(123456789).block());
	...
	public void findById_ValidId_CanFindUser() {
		...
		User findUserById_validId_User = userRepository.save(User.builder().displayName("findById_ValidId").build()).block();

		User returnedUser = userRepository.findById(findUserById_validId_User.getId()).block();

		assertNotNull(returnedUser);

		assertEquals(returnedUser, findUserById_validId_User);
