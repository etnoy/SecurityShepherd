...
class UserServiceTest {
	...
	void authenticate_EmptyPassword_ReturnsIllegalArgumentException() {
		StepVerifier.create(userService.authenticate("username", "")).expectError(IllegalArgumentException.class)
				.verify();
	}

	@Test
	void authenticate_EmptyUsername_ReturnsIllegalArgumentException() {
		StepVerifier.create(userService.authenticate("", "password")).expectError(IllegalArgumentException.class)
				.verify();
	}
	...
	void authenticate_NullPassword_ReturnsNullPointerException() {
		StepVerifier.create(userService.authenticate("username", null)).expectError(NullPointerException.class)
				.verify();
	}

	@Test
	void authenticate_NullUsername_ReturnsNullPointerException() {
		StepVerifier.create(userService.authenticate(null, "password")).expectError(NullPointerException.class)
				.verify();
	}
	...
	void authenticate_ValidUsernameAndPassword_ReturnsTrue() {
		...
		when(passwordAuthRepository.findByLoginName(mockedLoginName)).thenReturn(Mono.just(mockedPasswordAuth));
		when(mockedPasswordAuth.getHashedPassword()).thenReturn(mockedPasswordHash);
		StepVerifier.create(userService.authenticate(mockedLoginName, mockedPassword)).expectNext(true).expectComplete()
				.verify();
		verify(passwordAuthRepository, times(1)).findByLoginName(mockedLoginName);
	}
	...
	void authenticate_ValidUsernameButInvalidPassword_ReturnsTrue() {
		...
		when(passwordAuthRepository.findByLoginName(mockedLoginName)).thenReturn(Mono.just(mockedPasswordAuth));
		when(mockedPasswordAuth.getHashedPassword()).thenReturn(mockedPasswordHash);
		StepVerifier.create(userService.authenticate(mockedLoginName, wrongPassword)).expectNext(false).expectComplete()
				.verify();
		verify(passwordAuthRepository, times(1)).findByLoginName(mockedLoginName);
	}
	...
	void create_EmptyArgument_ReturnsIllegalArgumentException() {
		StepVerifier.create(userService.create("")).expectError(IllegalArgumentException.class).verify();
	}
	...
	void create_ValidDisplayName_CreatesUser() {
		...
		StepVerifier.create(userService.create(displayName)).expectNext(mockUserId).expectComplete().verify();
	...
	void deleteById_InvalidUserId_ReturnsInvalidUserIdException() {
		for (final Long userId : TestUtils.INVALID_IDS) {
			StepVerifier.create(userService.deleteById(userId)).expectError(InvalidUserIdException.class).verify();
		}
	}
	...
	void deleteById_ValidUserId_CallsRepository() {
		...
		final InOrder deletionOrder = inOrder(passwordAuthRepository, userAuthRepository, userRepository);
	...
	void demote_InvalidUserId_ReturnsInvalidUserIdException() {
		for (final Long userId : TestUtils.INVALID_IDS) {
			StepVerifier.create(userService.demote(userId)).expectError(InvalidUserIdException.class).verify();
		}
	}
	...
	void findAll_UsersExist_ReturnsUsers() {
		...
		StepVerifier.create(userService.findAll()).expectNext(mockUser1).expectNext(mockUser2).expectNext(mockUser3)
				.expectComplete().verify();
	...
	void findById_ExistingUserId_ReturnsUserEntity() {
		...
		StepVerifier.create(userService.findById(mockUserId)).expectNext(mockUser).expectComplete().verify();
	...
	void findById_InvalidUserId_ReturnsInvalidUserIdException() {
		for (final long userId : TestUtils.INVALID_IDS) {
			StepVerifier.create(userService.findById(userId)).expectError(InvalidUserIdException.class).verify();
		}
	}
	...
	void findByLoginName_EmptyLoginName_ReturnsIllegalArgumentException() {
		StepVerifier.create(userService.findUserIdByLoginName("")).expectError(IllegalArgumentException.class).verify();
	}
	...
	void findByLoginName_NullLoginName_ReturnsNullPointerException() {
		StepVerifier.create(userService.findUserIdByLoginName(null)).expectError(NullPointerException.class).verify();
	}
	...
	void findByLoginName_UserExists_ReturnsUser() {
		...
		StepVerifier.create(userService.findUserIdByLoginName(loginName)).expectNext(mockUserId).expectComplete()
				.verify();
		verify(passwordAuthRepository, times(1)).findByLoginName(loginName);
	}
	...
	void findDisplayNameById_InvalidUserId_ReturnsInvalidUserIdExceptio() {
		for (final long userId : TestUtils.INVALID_IDS) {
			StepVerifier.create(userService.findDisplayNameById(userId)).expectError(InvalidUserIdException.class)
					.verify();
		}
	}
	...
	void findDisplayNameById_UserExists_ReturnsDisplayName() {
		...
		StepVerifier.create(userService.findDisplayNameById(mockUserId)).expectNext(mockDisplayName).expectComplete()
				.verify();
	...
	void findPasswordAuthByLoginName_EmptyLoginName_ReturnsIllegalArgumentException() {
		StepVerifier.create(userService.findPasswordAuthByLoginName("")).expectError(IllegalArgumentException.class)
				.verify();
	}
	...
	void findPasswordAuthByLoginName_ExistingLoginName_ReturnsPasswordAuth() {
		...
		when(passwordAuthRepository.findByLoginName(mockLoginName)).thenReturn(Mono.just(mockPasswordAuth));

		StepVerifier.create(userService.findPasswordAuthByLoginName(mockLoginName)).expectNext(mockPasswordAuth)
				.expectComplete().verify();
	...
	void findPasswordAuthByLoginName_NonExistentLoginName_ReturnsEmpty() {
		...
		StepVerifier.create(userService.findPasswordAuthByLoginName(mockLoginName)).expectComplete().verify();
		verify(passwordAuthRepository, times(1)).findByLoginName(mockLoginName);
	}

	@Test
	void findPasswordAuthByLoginName_NullLoginName_ReturnsNullPointerException() {
		StepVerifier.create(userService.findPasswordAuthByLoginName(null)).expectError(NullPointerException.class)
				.verify();
	}
	...
	void findPasswordAuthByUserId_InvalidUserId_ReturnsInvalidUserIdException() {
		for (final long userId : TestUtils.INVALID_IDS) {
			StepVerifier.create(userService.findPasswordAuthByUserId(userId)).expectError(InvalidUserIdException.class)
					.verify();
		}
	}
	...
	void findPasswordAuthByUserId_PasswordAuthExists_ReturnsPasswordAuth() {
		...
		StepVerifier.create(userService.findPasswordAuthByUserId(mockUserId)).expectNext(mockPasswordAuth)
				.expectComplete().verify();
	...
	void findUserAuthByUserId_ExistingUserId_ReturnsUserAuth() {
		...
		StepVerifier.create(userService.findUserAuthByUserId(mockUserId)).expectNext(mockUserAuth).expectComplete()
				.verify();
	...
	void findUserAuthByUserId_InvalidUserId_ReturnsInvalidUserIdException() {
		for (final long userId : TestUtils.INVALID_IDS) {
			StepVerifier.create(userService.findUserAuthByUserId(userId)).expectError(InvalidUserIdException.class)
					.verify();
		}
	}
	...
	void findUserAuthByUserId_NonExistentUserId_ReturnsEmpty() {
		...
		StepVerifier.create(userService.findUserAuthByUserId(nonExistentUserId)).expectComplete().verify();
	...
	void getAuthoritiesByUserId_InvalidUserId_ReturnsInvalidUserIdException() {
		for (final long userId : TestUtils.INVALID_IDS) {
			StepVerifier.create(userService.getAuthoritiesByUserId(userId)).expectError(InvalidUserIdException.class)
					.verify();
		}
	}
	...
	void getKeyById_InvalidUserId_ReturnsInvalidUserIdException() {
		for (final long userId : TestUtils.INVALID_IDS) {
			StepVerifier.create(userService.findKeyById(userId)).expectError(InvalidUserIdException.class).verify();
		}
	}
	...
	void getKeyById_KeyExists_ReturnsKey() {
		...
		StepVerifier.create(userService.findKeyById(mockUserId)).expectNext(testRandomBytes).expectComplete().verify();
	...
	void promote_InvalidUserId_ReturnsInvalidUserIdException() {
		for (final long userId : TestUtils.INVALID_IDS) {
			StepVerifier.create(userService.promote(userId)).expectError(InvalidUserIdException.class).verify();
		}
	}
	...
	void setClassId_InvalidClassId_ReturnsInvalidClassIdException() {
		for (final long classId : TestUtils.INVALID_IDS) {
			StepVerifier.create(userService.setClassId(10L, classId)).expectError(InvalidClassIdException.class)
					.verify();
		}
	}
	...
	void setClassId_InvalidUserId_ReturnsInvalidUserIdException() {
		for (final long userId : TestUtils.INVALID_IDS) {
			StepVerifier.create(userService.setClassId(userId, 61)).expectError(InvalidUserIdException.class).verify();
		}
	}
	...
	void setClassId_NonExistentClassId_ReturnsClassIdNotFoundException() {
		...
		StepVerifier.create(userService.setClassId(mockUserId, mockClassId)).expectError(ClassIdNotFoundException.class)
				.verify();
	...
	void setDisplayName_EmptyDisplayName_ReturnsIllegalArgumentException() {
		StepVerifier.create(userService.setDisplayName(725, "")).expectError(IllegalArgumentException.class).verify();
	}
	...
	void setDisplayName_NullDisplayName_ReturnsNullPointerException() {
		StepVerifier.create(userService.setDisplayName(480, null)).expectError(NullPointerException.class).verify();
	}
	...
	private void setUp() {
		...
		userService = new UserService(userRepository, userAuthRepository, passwordAuthRepository, classService,
				keyService);
	}
}
